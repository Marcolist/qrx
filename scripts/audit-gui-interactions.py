...
"upscaler/index.html":["chooseMediaBtn","processBtn","upscaler_start_local","upscaler_job_status","upscaler_cancel_job","upscaler_video_pipeline_plan"],
...
for cmd in ["open_browser_www_window","upscaler_start_local","upscaler_job_status","upscaler_cancel_job","upscaler_video_pipeline_plan","open_aura_window","open_generals_window","open_qrx_browser_window","open_qrx_upscaler_window"]:
 if rs.count(cmd)<2: errors.append(f"Rust command not both defined/registered: {cmd}")
# 0.0.9.69 regression guard: the legacy synchronous upscaler bridge must stay removed.
legacy="upscaler_"+"run_local"
if legacy in rs or legacy in (src/"upscaler/index.html").read_text(errors="replace"):
 errors.append("legacy synchronous upscaler bridge reintroduced; use async job API")
