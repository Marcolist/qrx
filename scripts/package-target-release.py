...
manifest={"format":"QRX_MULTI_TARGET_RELEASE_V1","qrx_version":"0.0.9","target":args.target,"created_at_utc":datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),"files":records}
...
try:
    with zipfile.ZipFile(tmp_name,"w",zipfile.ZIP_DEFLATED,compresslevel=9) as z:
        for p in sorted(x for x in root.rglob("*") if x.is_file()):
            rel=Path(f"qrx-0.0.9-genesis-{args.target}")/p.relative_to(root)
