...
int main(void){
 QrxAdRewardSplit sp; assert(qrx_ad_reward_split(1000,&sp)==0); assert(sp.delivery_atoms==550&&sp.publisher_atoms==250&&sp.viewer_atoms==100&&sp.protocol_atoms==50&&sp.development_atoms==50);
 ...
 char v[64]; assert(qrxdb_get(&db,"qrxnet/ad/reward/qrx1viewer",v,sizeof(v))==0&&strtoull(v,0,10)==100); assert(qrxdb_get(&db,"qrxnet/ad/reward/qrx1publisher",v,sizeof(v))==0&&strtoull(v,0,10)==250);
 assert(qrx_net_consensus_prepare(dir,"AD_REWARD_CLAIM","qrx1viewer","qrx1viewer",0,"-","claim",151,&e)==0&&e.self_credit_atoms==100); stage(&db,dir,"AD_REWARD_CLAIM","qrx1viewer","qrx1viewer",0,"-","claim",151); assert(qrxdb_get(&db,"qrxnet/ad/reward/qrx1viewer",v,sizeof(v))==0&&strtoull(v,0,10)==0);
 assert(qrxdb_close(&db)==0); puts("PASS: QRX Ad Shield uses escrowed campaigns, 3-provider receipts, replay/frequency caps, and 55/25/10/5/5 rewards"); return 0;
}
