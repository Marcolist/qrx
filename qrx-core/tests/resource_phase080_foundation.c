...
int main(void) {
    ...
    /* Mainnet is fail-closed until a post-Genesis protocol-9 DRIVE_V1 schedule exists. */
    {
        ...
        assert(qrx_resource_drive_v1_scheduled(root) == 1);
        assert(qrx_resource_activation_height(root) == 700000);
        assert(qrx_resource_protocol_enabled_at_height(root, 699999) == 0);
        assert(qrx_resource_protocol_enabled_at_height(root, 700000) == 1);
        assert(qrx_resource_qrx_net_v1_scheduled(root) == 1);
        assert(qrx_net_activation_height(root) == 700000);
        assert(qrx_net_protocol_enabled_at_height(root, 699999) == 0);
        assert(qrx_net_protocol_enabled_at_height(root, 700000) == 1);
    }
