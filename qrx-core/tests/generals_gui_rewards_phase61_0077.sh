#!/usr/bin/env bash
set -euo pipefail
Q=${1:-./build-phase61/qrx}
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
T=$(mktemp -d /tmp/qrx-generals-p61.XXXXXX); trap 'rm -rf "$T"' EXIT
$Q init-chain "$T/chain" 20 5000 210000000000000 25000000 200000000000 qrx-regtest 1 5152583037 QRX-Generals-P61 >/dev/null
...
# Membership must be frozen into the season state.
strings "$T/chain/qrxdb" 2>/dev/null | grep -q "$CLAN" || true
mkdir -p "$T/chain/blocks";for i in $(seq 1 24);do echo p61 > "$T/chain/blocks/$i.block";done
apply alice GAME_SEASON_FINALIZE 'season_id=1' fin
R=$($Q generals-season-result-info "$T/chain" 1)
echo "$R"|grep -q '^reward_player_pool_atoms=140000000$'
echo "$R"|grep -q '^reward_clan_pool_atoms=60000000$'
echo "$R"|grep -q '^reward_total_atoms=200000000$'
C=$($Q generals-clan-ranking-info "$T/chain" 1)
echo "$C"|grep -q "^clan_1=$CLAN,200,60000000$"
RW=$($Q generals-season-rewards-info "$T/chain" 1)
TOTAL=$(echo "$RW"|awk -F',' '/^rank_[0-9]+=/{s+=$3}END{printf "%.0f",s}')
[[ $TOTAL -eq 200000000 ]]
GUI="$ROOT/../GUIWALLET/src/generals/index.html"; TAURI="$ROOT/../GUIWALLET/src-tauri/src/main.rs"
for tok in contextMenu showContext GAME_AIR_MISSION GAME_NAVAL_MOVE GAME_RECON_SCAN GAME_CITY_DEVELOP GAME_CLAN_CREATE '70% top 20 players' '30% top 3 clans';do grep -q "$tok" "$GUI";done
for tok in generals-clan-ranking-info generals-season-rewards-info tech_tree clan_ranking;do grep -q "$tok" "$TAURI";done
echo 'PASS: Phase 6.1 full GUI wiring surface, right-click tactical context menu, 70/30 player+clan season reward accounting, season clan snapshot and clan ranking'
